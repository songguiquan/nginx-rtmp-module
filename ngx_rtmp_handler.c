...
static ngx_flag_t
ngx_rtmp_relative_timestamp(ngx_rtmp_session_t *s, ngx_rtmp_frame_t *frame, ngx_rtmp_header_t *lh)
{
    ...
    ngx_jitter_correct(s, lh, frame, ag);
...
ngx_rtmp_prepare_out_chain(ngx_rtmp_session_t *s, ngx_rtmp_frame_t *frame)
{
    ...
    mlen = 0;
    fmt = 0;
    relative = ngx_rtmp_relative_timestamp(s, frame, &lh);
	ngx_log_error(NGX_LOG_ERR, s->connection->log, 0,
		"lh->timestamp:%ul", lh.timestamp);
